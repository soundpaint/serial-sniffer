...
{
  int32_t value = strtol_l(nptr, endptr, base, Config::LOCALE_CONFIG_C);
...
Abstract_config_reader::parse_decimal_int8(const XMLCh *token)
{
  ...
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    ...
    full_msg << "Abstract_config_reader::parse_decimal_int8(): "
      "value is not a byte, error position: " <<
      (err_pos - token_as_c_star);
    Log::fatal(full_msg.str());
  }
  xercesc::XMLString::release(&token_as_c_star);
  return value;
...
Abstract_config_reader::parse_decimal_int16(const XMLCh *token)
{
  ...
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    ...
    full_msg << "Abstract_config_reader::parse_decimal_int16(): "
      "value is not a short, error position: " <<
      (err_pos - token_as_c_star);
    Log::fatal(full_msg.str());
  }
  xercesc::XMLString::release(&token_as_c_star);
...
Abstract_config_reader::parse_decimal_int32(const XMLCh *token)
{
  ...
  char *err_pos;
  const int32_t value =
    strtol_l(token_as_c_star, &err_pos, 10, Config::LOCALE_CONFIG_C);
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    std::stringstream full_msg;
    full_msg << "Abstract_config_reader::parse_decimal_int32(): "
      "value is not a long, error position: " <<
      (err_pos - token_as_c_star);
    Log::fatal(full_msg.str());
  }
  xercesc::XMLString::release(&token_as_c_star);
...
Abstract_config_reader::parse_decimal_uint32(const XMLCh *token)
{
  ...
  const uint32_t value =
    strtoul_l(token_as_c_star, &err_pos, 10, Config::LOCALE_CONFIG_C);
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    std::stringstream full_msg;
...
Abstract_config_reader::parse_hex_or_dec_or_oct_int32(const XMLCh *token)
{
  ...
  char *err_pos;
  const uint32_t value =
    strtoul_l(token_as_c_star, &err_pos, 0, Config::LOCALE_CONFIG_C);
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    std::stringstream full_msg;
...
Abstract_config_reader::parse_decimal_int64(const XMLCh *token)
{
  ...
  char *err_pos;
  const int64_t value =
    strtoll_l(token_as_c_star, &err_pos, 10, Config::LOCALE_CONFIG_C);
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    std::stringstream full_msg;
...
Abstract_config_reader::parse_decimal_uint64(const XMLCh *token)
{
  ...
  const uint64_t value =
    strtoull_l(token_as_c_star, &err_pos, 10, Config::LOCALE_CONFIG_C);
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    std::stringstream full_msg;
...
Abstract_config_reader::parse_float(const XMLCh *token)
{
  ...
  char *err_pos;
  const float value =
    strtof_l(token_as_c_star, &err_pos, Config::LOCALE_CONFIG_C);
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    std::stringstream full_msg;
    full_msg << "Abstract_config_reader::parse_float(): "
      "value is not a double, error position: " <<
      (err_pos - token_as_c_star);
    Log::fatal(full_msg.str());
  }
  xercesc::XMLString::release(&token_as_c_star);
...
Abstract_config_reader::parse_double(const XMLCh *token)
{
  ...
  char *err_pos;
  const double value =
    strtod_l(token_as_c_star, &err_pos, Config::LOCALE_CONFIG_C);
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    std::stringstream full_msg;
...
Abstract_config_reader::parse_long_double(const XMLCh *token)
{
  ...
  char *err_pos;
  const double value =
    strtold_l(token_as_c_star, &err_pos, Config::LOCALE_CONFIG_C);
  if (err_pos != token_as_c_star + strlen(token_as_c_star)) {
    std::stringstream full_msg;
                                                 ...
                                                 const bool required)
{
  ...
  if (!utf8_name) {
    Log::fatal("Abstract_config_reader::get_single_child_element(): "
               "not enough memory");
  }
  const xercesc::DOMNodeList *node_list =
...
{
  if (!path) {
    Log::fatal("Abstract_config_reader::parse(): path is NULL");
  }
  xercesc::XercesDOMParser *parser = new xercesc::XercesDOMParser();
  if (!parser) {
    Log::fatal("Abstract_config_reader::parse(): "
  ...
  try {
    ...
    delete exc;
    Log::fatal(full_msg.str());
  } catch (xercesc::DOMException *exc) {
    std::stringstream full_msg;
    char *message = xercesc::XMLString::transcode(exc->getMessage());
    full_msg << "Abstract_config_reader::Config(): "
      "failed parsing XML config file: " << message;
    ...
    delete exc;
    Log::fatal(full_msg.str());
  } catch (xercesc::SAXParseException *exc) {
    std::stringstream full_msg;
    char *message = xercesc::XMLString::transcode(exc->getMessage());
    ...
    delete exc;
    Log::fatal(full_msg.str());
  } catch (xercesc::SAXException *exc) {
    std::stringstream full_msg;
    char *message = xercesc::XMLString::transcode(exc->getMessage());
  ...
  if (document) {
    Log::info("Abstract_config_reader::parse(): XSD successfully read");
  } else {
    Log::fatal("Abstract_config_reader::parse(): "
               "document is NULL (maybe XML file does not exist?)");
  }
  const xercesc::DOMElement *elem_config = document->getDocumentElement();
  if (!elem_config) {
    Log::fatal("Abstract_config_reader::parse(): document element is NULL");
  }
  ...
  // Do *not* call "document->release()", since the document is
